...
axes.plot(x,ibl_catch_cue_wass,color='black',marker='+',dashes=[0,0,2,2],label='ACT-R',linewidth=1)
axes.legend()

axes.set_xlabel('Retrieval Cue', labelpad=2,size=12)
axes.set_ylabel('Wasserstein Distance', labelpad=1,size=12)
axes.set_ylim([0,1])
...
for x in range(0,2):
    for y in range(0,5):
        ...
        axes[x,y].text(500,90,cue_list[counter],fontsize=8)
        counter = counter+1
        #print(x,' ',y,' ',counter)
...
axes[1,4].set_xlabel('Retrieval Patt.', labelpad=10,size=9)

axes[1,0].tick_params(axis='x', labelsize=8)
axes[1,1].tick_params(axis='x', labelsize=8)
axes[1,2].tick_params(axis='x', labelsize=8)
axes[1,3].tick_params(axis='x', labelsize=8)
axes[1,4].tick_params(axis='x', labelsize=8)
