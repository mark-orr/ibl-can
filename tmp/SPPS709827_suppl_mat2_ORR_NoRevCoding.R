...
#########################
#Load data 
ANES2012 <- read.dta('35157-0001-Data.dta')#loads the data to the object ANES2012
...
write.csv(Obama,file="Obama.csv",quote=FALSE,sep=',',row.names=FALSE)
...
write.csv(SampleBeta10,file="can_beta_10.csv",quote=FALSE,sep=',',row.names=FALSE)

SampleBeta1 <- IsingSampler(n=100, graph=SimInput$weiadj, 
                             thresholds=SimInput$thresholds,beta=1,
                             responses = c(0L,1L))

write.csv(SampleBeta1,file="can_beta_1.csv",quote=FALSE,sep=',',row.names=FALSE)

SampleBeta005 <- IsingSampler(n=100, graph=SimInput$weiadj, 
                             thresholds=SimInput$thresholds,beta=0.05,
                             responses = c(0L,1L))

write.csv(SampleBeta005,file="can_beta_005.csv",quote=FALSE,sep=',',row.names=FALSE)
...
for (i in beta.list){
  ...
  assign(paste("SampleBeta_",i,sep=""),IsingSampler(n=100, graph=SimInput$weiadj, 
                               ...
                               responses = c(0L,1L)))
  write.csv(get(paste("SampleBeta_",i,sep="")),file=paste("can_beta_",i,".csv",sep=""),quote=FALSE,sep=',',row.names=FALSE)
}
